...
def _larfb_kernel_mx(
    ...
    SOLVE: tl.constexpr, CH: tl.constexpr,
):
    ...
    for t in range(0, (M + CH - 1) // CH):
        ...
        Vc = tl.load(Vb + v_off, mask=rmask[:, None] & (col_idx[None, :] < ib), other=0.0)
...
def _tsqr_apply_kernel_mx(
    ...
    FOLD_TREE: tl.constexpr, BRMt: tl.constexpr, NO_DOT: tl.constexpr,
):
    pid_b = tle.program_id(0)
...
def _assemble_q_single_panel_kernel_mx(
    ...
    Vrows = tl.load(Vb + p_idx[:, None] * sVm + col_idx[None, :] * sVn,
                    mask=(p_idx[:, None] < m) & (col_idx[None, :] < k), other=zero)
    Z = tl.trans(Vrows)
    # M = T^{-1} (upper triangular, IBN x IBN)
    Msram = tl.load(Tb + col_idx[:, None] * sTm + col_idx[None, :] * sTn,
                    mask=(col_idx[:, None] < k) & (col_idx[None, :] < k), other=zero)
    ...
    Vt = tl.load(Vb + rows[:, None] * sVm + col_idx[None, :] * sVn,
                 mask=rmask[:, None] & (col_idx[None, :] < k), other=zero)
...
def _larft_gram_partial_kernel_mx(
    V, Gpart, M, ib,
    sVb, sVm, sVn, sGb, sGr, sGm, sGn,
    GRAN: tl.constexpr, IBN: tl.constexpr, CH: tl.constexpr,
):
...
def _larft_finalize_kernel_mx(
    Gpart, TAU, MOUT, RB, ib,
    sGb, sGr, sGm, sGn, sTauB, sTauN, sMb, sMm, sMn,
    IBN: tl.constexpr,
):
    ...
    Mmat = tl.where(idx[:, None] == idx[None, :], inv_tau[:, None], Mmat)
    tl.store(
        Tb + idx[:, None] * sMm + idx[None, :] * sMn, Mmat,
        mask=(idx[:, None] < ib) & (idx[None, :] < ib),
    )
...
def _larfb_w1_partial_kernel_mx(
    V, C, Wpart, M, ib, P,
    sVb, sVm, sVn, sCb, sCm, sCn, sWb, sWr, sWp, sWm, sWn,
    GRAN: tl.constexpr, IBN: tl.constexpr, TN: tl.constexpr, CH: tl.constexpr,
):
...
def _larfb_solve_kernel_mx(
    TOUT, Wpart, Yout, RB, ib, P,
    sTb, sTm, sTn, sWb, sWr, sWp, sWm, sWn, sYb, sYp, sYm, sYn,
    IBN: tl.constexpr, TN: tl.constexpr, UPPER: tl.constexpr,
):
...
def _larfb_apply_kernel_mx(
    V, Y, C, M, ib, P,
    sVb, sVm, sVn, sYb, sYp, sYm, sYn, sCb, sCm, sCn,
    GRAN: tl.constexpr, IBN: tl.constexpr, TN: tl.constexpr, CH: tl.constexpr,
):
...
def _geqrt_sram_t_kernel_mx(
    W, V, TAU, TBUF, M, ib, kk, n, k,
    sWb, sWm, sWn, sVb, sVm, sVn, sTauB, sTauN, sTb, sTm, sTn,
    ...
    for j in range(ib):
        ...
        pair = tl.sum(tl.join(tl.where(rm == j, col_j, zero),
                              tl.where(rm > j, col_j * col_j, zero)), axis=0)
    ...
    tl.store(TBUF + pid * sTb + (kk + cn)[:, None] * sTm + (kk + cn)[None, :] * sTn,
             Tmat, mask=cmask[:, None] & cmask[None, :])
...
def _launch_geqrt_sram_t_mx(W, V, tau, Tbuf, m, n, k, kk, ib, B):
    ...
    _geqrt_sram_t_kernel_mx.fn[(B,)](
        W, V, tau, Tbuf, M, ib_active, kk, n, k,
        sWb, sWm, sWn, sVb, sVm, sVn, sTauB, sTauN, sTb, sTm, sTn,
        BM=triton.next_power_of_2(M), IBN=max(16, triton.next_power_of_2(ib_active)),
...
def _launch_geqrt_mcta_mx(W, V, tau, alpha_buf, xnorm_buf, w_sum, rowj_buf, ctr,
                          m, n, k, kk, ib, p, NC, B, rm=_MCTA_RM):
    ...
    _geqrt_mcta_kernel.fn[(B, NC)](
        W, V, tau, alpha_buf, xnorm_buf, w_sum, rowj_buf, ctr,
        M, kk, ib, n, k, nr, p,
        sWb, sWm, sWn, sVb, sVm, sVn, sTauB, sTauN,
        sAB, sAM, sXB, sXM, sWB, sWM, sWN, sRB, sRM2, sRN2, sCtrB, sCtrM,
        CHUNK=CHUNK, RM=rm, IBN=max(16, triton.next_power_of_2(ib)), NC=NC, NSYNC=NSYNC,
        NUM_TILES=NUM_TILES,
    ...
    if V.element_size() == 8:
        ...
        if M > _PAR_GRAN:
            ...
            _larft_gram_partial_kernel_mx[(B, rb)](
                V, Gpart, M, ib,
                sVb, sVm, sVn, *Gpart.stride(),
                GRAN=_PAR_GRAN, IBN=ibn, CH=_bcast_ch(ibn, ibn),
                num_warps=_LARFT_WARPS if warps is None else warps,
                num_stages=1,
            )
            _larft_finalize_kernel_mx[(B,)](
                Gpart, tau, Tout, rb, ib,
                *Gpart.stride(), sTauB, sTauN, sTb, sTm, sTn,
                IBN=ibn,
                num_warps=_LARFT_WARPS if warps is None else warps,
                num_stages=1,
            )
        ...
        _larft_kernel_mx[(B,)](
            ...
            RM=_PANEL_RM, IBN=ibn, CH=_bcast_ch(ibn, ibn),
            num_warps=_LARFT_WARPS if warps is None else warps,
        ...
        _larft_kernel.fn[(B,)](
            V, tau, Tout, M, ib, sVb, sVm, sVn, sTauB, sTauN, sTb, sTm, sTn,
            RM=_PANEL_RM, IBN=max(16, triton.next_power_of_2(ib)),
            INVERT=True,
            num_warps=_LARFT_WARPS if warps is None else warps,
        )
...
def _launch_larfb_mx(V, tau, Tp, C, m, p, ib, B, upper):
    ...
    if V.element_size() == 8:
        ...
        if m > _PAR_GRAN:
            ...
            rb = (m + _PAR_GRAN - 1) // _PAR_GRAN
            Wpart = torch.empty(B, rb, grid_p, ibn, tn, dtype=V.dtype,
                                device=V.device)
            sWb, sWr, sWp, sWm, sWn = Wpart.stride()
            Ybuf = torch.empty(B, grid_p, ibn, tn, dtype=V.dtype,
                               device=V.device)
            sYb, sYp, sYm, sYn = Ybuf.stride()
            _larfb_w1_partial_kernel_mx[(B, rb, grid_p)](
                V, C, Wpart, m, ib, p,
                sVb, sVm, sVn, sCb, sCm, sCn, sWb, sWr, sWp, sWm, sWn,
                GRAN=_PAR_GRAN, IBN=ibn, TN=tn, CH=_bcast_ch(ibn, tn),
                num_warps=_LARFB_WARPS,
                num_stages=1,
            )
            _larfb_solve_kernel_mx[(B, grid_p)](
                Tp, Wpart, Ybuf, rb, ib, p,
                sTb, sTm, sTn, sWb, sWr, sWp, sWm, sWn, sYb, sYp, sYm, sYn,
                IBN=ibn, TN=tn, UPPER=upper,
                num_warps=_LARFB_WARPS,
                num_stages=1,
            )
        ...
        _larfb_kernel_mx[(B, grid_p)](
            V, tau, Tp, C, m, ib, p,
            sVb, sVm, sVn, sTauB, sTauN, sTb, sTm, sTn, sCb, sCm, sCn,
            RM=_LARFB_RM, IBN=ibn, TN=tn,
            UPPER=upper, SOLVE=True, CH=_bcast_ch(ibn, tn),
            num_warps=_LARFB_WARPS,
            num_stages=1,
        )
    else:
        _larfb_kernel[(B, grid_p)](
            V, tau, Tp, C, m, ib, p,
            sVb, sVm, sVn, sTauB, sTauN, sTb, sTm, sTn, sCb, sCm, sCn,
            RM=_LARFB_RM, IBN=max(16, triton.next_power_of_2(ib)), TN=tn,
            UPPER=upper, SOLVE=False,
            num_warps=_LARFB_WARPS,
...
def _launch_tsqr_apply(grid, V_local, TAU_local, Qt, Qt2, Q, Rblocks, Racc,
                       m, n, br, k_max, grp, num_blocks,
                       sQb, sQm, sQn, sRAb, sRAm, sRAn,
                       BM, IBN, TWO_LEVEL, FOLD_TREE, BRMt, num_warps):
    if Q.element_size() == 8:
        ...
        _tsqr_apply_kernel_mx[grid](
            ...
            FOLD_TREE=FOLD_TREE, BRMt=BRMt, NO_DOT=True,
            num_warps=num_warps,
        )
    else:
...
def _blocked_qr_mx(W, V, tau, Tbuf, m, n, k, ib=None):
    ...
    for kk in range(0, k, ib):
        ...
        if _panel_has_fused_t(W.element_size(), m, kk, ib):
            ...
            if nc >= _MCTA_MIN_NC:
                _launch_geqrt_mcta_mx(W, V, tau, alpha_buf, xnorm_buf, w_sum, rowj_buf, ctr,
                                      m, n, k, kk, ib_active, kk // ib, nc, B, rm=rm)
            else:
        ...
        Tp = torch.as_strided(Tbuf, (B, ib_active, ib_active), (sTb_, sTm_, sTn_),
                              soTb + kk * sTm_ + kk * sTn_)
        if kk + ib_active < n:
            if not fused_t:
                _launch_larft_mx(Vp, taup, Tp, m, kk, ib_active, B)
            C = torch.as_strided(W, (B, M, n - (kk + ib_active)), sW,
                                 soW + kk * sW[1] + (kk + ib_active) * sW[2])
            _launch_larfb_mx(Vp, taup, Tp, C, m - kk, n - (kk + ib_active),
                             ib_active, B, upper=False)
...
def _tsqr_mx(W, m, n, k, mode, B, out_Q=None, out_R=None):
    ...
    flat = BRM * IBNt <= red_elem and BRM * IBNt <= _TSQR_MX_FLAT_FP32 \
    ...
    _tsqr_local_sram_kernel[(B, num_blocks)](
        W, R_blocks, V_local, TAU_local, m, n, br, num_blocks, k_max,
        sWb, sWm, sWn,
        R_blocks.stride(0), R_blocks.stride(1), R_blocks.stride(2),
        sVb, sVm, sVn, TAU_local.stride(0),
        BM=BM, IBN=IBN, STORE_V=write_Q,
...
def _assemble_q_mx(V, tau, Tbuf, m, n, k, qcols, ib, B, out):
    ...
    if P == 1 and (V.element_size() == 4
                   or (k <= _ASSEMBLE_1P_MAX_K_FP64 and m >= n)):
        ...
        if V.element_size() == 4:
            _assemble_q_single_panel_kernel[(B, grid_p)](
                Vp, Tp, out, m, k, qcols,
                sVb, sVm, sVn, sTb, sTm, sTn, sQb, sQm, sQn,
                RM=_ASSEMBLE_RM, TN=_ASSEMBLE_TN, IBN=ibn,
    ...
    for kk in reversed(range(0, k, ib)):
        ...
        _launch_larfb_mx(Vp, taup, Tp, out[:, kk:m, :], m - kk, qcols,
...
def _linalg_qr_mx(A, mode="reduced", *, out=None):
    ...
    V = torch.zeros(B, m, k, dtype=W.dtype, device=W.device) if mode != "r" \
