...
from flag_gems.ops.linalg_qr import _FUSED_DIM, _FUSED_ELEM, _FUSED_M, _FUSED_TALL_M
from flag_gems.ops.linalg_qr import _PANEL_IB, _TSQR_ASPECT, _TSQR_MAX_N, _TSQR_MIN_M
...
def _use_safe_q_assembly(A, mode):
    ...
    is_ts = (m >= _TSQR_ASPECT * n) and (m >= _TSQR_MIN_M) \
        and (n <= _TSQR_MAX_N) and mode == "reduced"
    fits_fused = (m <= _FUSED_M and n <= _FUSED_DIM and m * n <= _FUSED_ELEM
