...
def _batch_chunks(B):
    ...
    if B <= _MAX_BLOCKS:
        return ((0, B),)
    return tuple(
        (b0, min(_MAX_BLOCKS, B - b0)) for b0 in range(0, B, _MAX_BLOCKS)
    )
...
def _copy_a_to_wc(A, Wc, m, n, ld, sAb, sAm, sAn, sWb, RM: tl.constexpr):
    ...
    for t in range(tl.program_id(2), tl.cdiv(m, RM), tl.num_programs(2)):
        ...
        rmask = rows < m
        v = tl.load(A + pid_b * sAb + j * sAn + rows * sAm, mask=rmask,
                    other=0.0)
...
# ---------------------------------------------------------------------------
@triton.jit
def _panel_kernel(Wc, Vc, TAU, R, m, j0, nb, ld,
                  sWb, sVb, sTauB, sTauN, sRb, sRm, sRn,
    ...
    if WRITE_R:
        ...
        rv = tl.load(Wb + rc[None, :] * ld + rr[:, None],
                     mask=rmask2, other=zero)
        tl.store(R + pid_b * sRb + rr[:, None] * sRm + rc[None, :] * sRn,
                 rv * (rr[:, None] <= rc[None, :]), mask=rmask2)
...
# ---------------------------------------------------------------------------
@triton.jit
def _panel_mcta_kernel(Wc, Vc, TAU, ABUF, XBUF, WBUF, CTR, R,
                       m, j0, nb, base, nc, ld,
                       sWb, sVb, sTauB, sTauN, sRb, sRm, sRn,
                       RM: tl.constexpr, PW: tl.constexpr,
                       WRITE_R: tl.constexpr):
...
# ---------------------------------------------------------------------------
@triton.jit
def _copy_qc_to_q(Qc, Q, m, qcols, ld, sQcb, sQb, sQm, sQn,
    ...
    v = tl.load(Qc + pid_b * sQcb + rows[:, None] + cols[None, :] * ld,
                mask=mask, other=0.0)
    tl.store(Q + pid_b * sQb + rows[:, None] * sQm + cols[None, :] * sQn, v,
             mask=mask)


@triton.jit
def _triu_copy_cm(Wc, R, rrows, n, ld, sWb, sRb, sRm, sRn,
                  TM: tl.constexpr, TN: tl.constexpr):
    pid_b = tl.program_id(0)
    bi = tl.program_id(1)
    bj = tl.program_id(2)
    ...
    v = tl.load(Wc + pid_b * sWb + rows[:, None] + cols[None, :] * ld,
                mask=mask, other=0.0)
    tl.store(R + pid_b * sRb + rows[:, None] * sRm + cols[None, :] * sRn,
             v * (rows[:, None] <= cols[None, :]), mask=mask)
...
def _ascend_linalg_qr(A, mode, out=None):
    ...
    with torch_device_fn.device(A.device):
        ...
        if (
            ...
            for li, (rows_l, P_l) in enumerate(lvls):
                ...
                for u0, un in _axis1_chunks(P_l, B):
                    _tsqr_l1_kernel[(B, un)](src_t, dstR, V1s[li], T1s[li],
                        rows_l, n, mp, u0,
            ...
            if l2_reg:
                ...
                ldv2 = 128
                Vc2r = torch.empty(B, n, ldv2, dtype=A.dtype,
                                   device=A.device)
                ...
                for b0, bn in _batch_chunks(B):
                    Ss, V2s, T2s, Q2s, Rs = _bslice(
                        (stack, Vc2r, tau2r, Q2, R), b0, bn)
                    _qr_reg_kernel[(bn,)](Ss, V2s, Q2s, Rs, T2s,
                        m2, n, n, qc2, n, ldv2,
                        sSb2, sSm2, sSn2, n * ldv2,
                        sQ2b, sQ2m, sQ2n, sRb, sRm, sRn,
                        RM=RM2, TN=TN2, TQ=TQ2, PUT_Q=bool(qc2))
                if mode == "r":
                ...
                _panel_mcta_kernel[(B, nc2)](Wc2, Vc2, tau2, ab2, xb2, wb2, ct2,
                    R, m2, 0, n, 0, nc2, ld2,
                    n * ld2, n * ld2, n, 1, sRb, sRm, sRn,
                    RM=64, PW=64, WRITE_R=True)
            else:
                _panel_kernel[(B,)](Wc2, Vc2, tau2, R, m2, 0, n,
                              ld2, n * ld2, n * ld2, n, 1, sRb, sRm, sRn,
                              RM=_RM, PW=tn1, WRITE_R=True)
            if mode == "r":
                Q = out_Q if out_Q is not None else torch.empty(
                    (0,), dtype=A.dtype, device=A.device)
                return Q, R.reshape(*batch_shape, n, n)
            ...
            for li in range(len(lvls) - 1, -1, -1):
                ...
                for u0, un in _axis1_chunks(P_l, B):
                    _tsqr_q_kernel[(B, un)](Xs[li], V1s[li], T1s[li], dst,
                        rows_l, n, mp, u0,
        ...
        if reg is not None:
            ...
            for b0, bn in _batch_chunks(B):
                ...
                _qr_reg_kernel[(bn,)](As, Vs, Qs, Rs, Ts,
        ...
        ws = torch.empty(B * (n * ld + k * ld + k + qcols * ld) + extra,
        ...
        for b0, bn in _batch_chunks(B):
            ...
            _copy_a_to_wc[(bn, n, _cpy_tiles(n, m, _RM))](As, Ws, m, n, ld,
            ...
            for j0 in range(0, k, _NB):
                ...
                if use_mcta:
                    ...
                    _panel_mcta_kernel[(bn, nc)](Ws_, Vs_, Ts_, abs_, xbs_, wbs_, cs_,
                        Ws_, m, j0, nb, cbase, nc, ld,
                        sWb, sVb, sTauB, sTauN, 0, 0, 0, RM=64, PW=64,
                        WRITE_R=False)
                    cbase += 3 * nb * nc
                else:
                    _panel_kernel[(bn,)](Ws, Vs, Ts, Ws, m, j0, nb,
                                  ld, sWb, sVb, sTauB, sTauN, 0, 0, 0,
                                  RM=_RM, PW=_tile_width(nb), WRITE_R=False)
                c0 = j0 + nb
                p = n - c0
                if p > 0:
    ...
    if mode == "r":
        ...
        with torch_device_fn.device(A.device):
            for b0, bn in _batch_chunks(B):
                ...
                if sRn == 1:
                    _triu_copy_cm[(bn, _grid_elem(k, 64), _grid_elem(n, _TN))](Ws, Rs, k, n,
                                  ld, sWb, sRb, sRm, sRn, TM=64, TN=_TN)
                else:
                    _triu_copy_scalar[(bn,)](Ws, Rs, k, n, ld,
                                         sWb, sRb, sRm, sRn)
        ...
        for b0, bn in _batch_chunks(B):
            ...
            for u0, un in _axis1_chunks(_grid_elem(qcols, pw_q), bn):
                _q_apply_kernel[(bn, un)](
                          ...
                          RM=_RM, PW=pw_q)
            if sQn == 1:
                _copy_qc_to_q[(bn, _grid_elem(m, 64), _grid_elem(qcols, _TN))](Qcs, Qs,
                              m, qcols, ld, sQcb, sQb, sQm, sQn, TM=64, TN=_TN)
            else:
                _copy_qc_to_q_scalar[(bn,)](Qcs, Qs, m, qcols, ld,
                                     sQcb, sQb, sQm, sQn)
    ...
    with torch_device_fn.device(A.device):
        for b0, bn in _batch_chunks(B):
            Ws, Rs = _bslice((Wc, R), b0, bn)
            if sRn == 1:
                _triu_copy_cm[(bn, _grid_elem(rrows, 64), _grid_elem(n, _TN))](Ws, Rs,
                              rrows, n, ld, sWb, sRb, sRm, sRn, TM=64, TN=_TN)
            else:
                _triu_copy_scalar[(bn,)](Ws, Rs, rrows, n, ld,
                                     sWb, sRb, sRm, sRn)
